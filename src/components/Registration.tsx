  ...
  return (
    ...
    >
      <Wrap>
        ...
        {/* centred while stacked; on desktop a narrower centred pair of columns, each left-aligned */}
        <div className="mx-auto mt-11 grid items-start justify-items-center gap-11 text-center sm:mt-16 nav:max-w-240 nav:grid-cols-2 nav:justify-items-start nav:gap-10 nav:text-left">
          <Reveal>
            <div className="flex items-end justify-center gap-3.5 nav:justify-start">
              <span className={bigNumberClass}>{event.seats}</span>
              <span className={bigWordClass}>
                <Lines lines={c.seatsWord} />
              </span>
            </div>
            <p className="mx-auto mt-4 max-w-75 text-[13px] leading-[1.55] text-muted sm:text-sm nav:mx-0">
              {c.seatsBody}
            </p>
          </Reveal>
